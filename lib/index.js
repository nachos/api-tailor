          ...
          .then(function (interceptedRequest) {
            ...
            var handleResponse = function (responseObject) {
              return responses.reduce(Q.when, Q.resolve(responseObject))
                .then(function (interceptedResponse) {
                  if (interceptedResponse && interceptedResponse.response && interceptedResponse.response.statusCode >= 400) {
                    debug('Request failed with error code %s', interceptedResponse.response.statusCode);
                    debug('The original response object %j', responseObject);
                    deferred.reject(interceptedResponse);
            ...
            if (route.stream) {
              ...
              request(interceptedRequest, function (err, response, body) {
                ...
                handleResponse(responseObject)
                  .then(function (interceptedResponse) {
                    debug('Done, resolved %j', interceptedResponse);
                    interceptedResponse = interceptedResponse || {};
                    deferred.resolve(interceptedResponse.body || interceptedResponse);
                  });
              });
            }
